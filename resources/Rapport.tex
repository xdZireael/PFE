...
construisant une carte de son environnement, le \acrfull{slam} est devenu une
brique fondamentale de nombreuses applications: navigation en intérieur,
...
aux scènes peu texturées ou d'autres capteurs sont erronées. Enfin, VINS-Mono
et VINS-Fusion intègrent la vision et une centrale inertielle (\acrshort{imu})
...
\emph{keypoints}. Des avancées récentes comme le Thermal-SLAM qui reduit
l'impact du bruit thermique et améliore la détection de
...
compatible avec ROS~2 Jazzy et s'ils etaient facilement integrable dans le
projet.

\section{Système d'exploitation et middleware}
Le systeme d'exploitation ici ce fait de soit car pour assurer la bonne
continuite du projet et pour eviter les problemes de compatibilité il faut
choisir un systeme d'exploitation compatible avec ROS~2 Jazzy.
...
Le SLAM a ete implemente ``from scratch'' pour le LiDAR (ICP) et la calibration
...
\begin{description}
  ...
  \item[Bresnham] Utilisé pour trouver les correspondances entre les points du LiDAR et
        les pixels.
  \item[ORB-SLAM3] Utilisé pour la branche SLAM visuel pur (RGB) afin de disposer d'une
        référence académique reconnue.
\end{description}

\section{Calibration}
La calibration de la camera n'as pas ete l'objectif principal du projet alors
le choix d'utiliser une solution existante a été fait pour gagner du temps et
se concentrer sur le SLAM.\@

\begin{description}
  \item[camera\_calibration (ROS~2)] Calibration intrinsèque caméra via damier (modèle
...
bibliothèque \texttt{rpg\_trajectory\_evaluation} développée par l'Université
de Zurich, qui fournit des métriques standardisées (ATE, RPE).

\begin{description}
  \item[rpg\_trajectory\_evaluation] Pour l'evaluation de la precision de l'odométrie
        LiDAR via \url{https://github.com/uzh-rpg/rpg_trajectory_evaluation}
\end{description}
...
\begin{description}
  ...
  \item[Python 3.10 et OpenCV] Langage de programmation utilise pour la pipeline caméra
        avec OpenCV.\@
...
Tout les tests ont été réalisés dans mon bureau, une pièce de 10m$^2$.
Ci-dessous est un benchmark de la carte générée par les outils turtlebot~4.
...
\section{Résultats}
Les packages ROS2 developpés permettent une modularité des pipelines SLAM et
donc une visualisation cohérente dans RViz2. Ci dessous sont les cartes
obtenues par les pipelines SLAM RGB et LiDAR.\@

Pour chaque cas de test voici le parcours realisé par le robbot:
\begin{figure}[H]
  \centering
...
\begin{figure}[H]
  ...
  \caption{Carte SLAM pas de RGB loop closure dans RViz2}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Composants actifs du pipeline SLAM (sans LiDAR)}\label{tab:results-rgb}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{c c c c c}
    \toprule
    \textbf{Test ID} & \textbf{LiDAR} & \textbf{Loop Closure LiDAR} & \textbf{Loop Closure RGB} & \textbf{Fusion} \\
  ...
  \end{tabularx}
\end{table}
Pas besoin de figure pour ce test car la camera fait partie du pipeline uniquement pour la loop closure,
et la carte est principalement construite par le LiDAR.\@

\begin{table}[H]
  \centering
  \caption{Composants actifs du pipeline SLAM (sans ICP)}\label{tab:results-noicp}
  \renewcommand{\arraystretch}{1.3}
...
\begin{figure}[H]
  ...
  \caption{Carte SLAM pas de LiDAR loop closure dans RViz2}
\end{figure}
\chapter{Conclusion}
...
ainsi que le fonctionement des piplines SLAM en général. La plateforme
TurtleBot~4 s'est avérée simple à prendre en main, hors mis la mise en place de
l'environnement ROS. Le pipeline MC-SLAM développé sous ROS~2 montre la
difficulté de l'adaptation sur les séquences testées. Au final, je ne suis pas
content du résultat final, le projet a été plus difficile que prévu et je n'ai
pas réussi à faire fonctionner les pipelines SLAM de manière satisfaisante, que
ce soit en RGB ou en LiDAR. Par contre, j'ai appris énormément sur les
algorithmes de SLAM, la calibration multi-capteurs et les défis de
l'implémentation sur une plateforme embarquée.
...
\begin{itemize}
  ...
  \item \textbf{Intégration de la caméra LWIR}~: ajouter la prise en charge de la caméra thermique et développer un pipeline SLAM
        dédié pour exploiter ses données.
  \item \textbf{Création de la pipeline SLAM RGB}~: implémenter un pipeline SLAM pour avoir une reel comparaison.
\end{itemize}
