...
\title{
  \vspace{-1cm}
  \includegraphics[width=4cm]{UB.png}\\[1.5cm]  
...
Malgré la l'avancé des algorithmes de \acrshort{slam} monoculaires ou
...
caméras monoculaire, stéréo et RGB-D, avec une gestion de la
réinitialisation et des cartes multi-sessions~\cite{campos2021orbslam3}.
DSO (\emph{Direct Sparse Odometry}) adopte quant à lui une approche directe
fondée sur la minimisation de l'erreur photométrique, ce qui le rend
particulièrement adapté aux scènes peu texturées ou d'autres capteurs sont erronées.
Enfin, VINS-Mono et VINS-Fusion intègrent la vision et une centrale inertielle (\acrshort{imu})
afin de fournir une odométrie visuo-inertielle précise.

\section{SLAM LiDAR}

Les approches LiDAR offrent une précision métrique supérieure en extérieur
et dans les larges espaces. LOAM et sa variante légère LeGO-LOAM reposent
sur l'extraction de \emph{features} géométriques des arêtes et plans pour
assurer l'odométrie et la cartographie en temps réel. LIO-SAM étend cette
approche en fusionnant les données LiDAR et \acrshort{imu} au sein d'une
optimisation par graphe de facteurs. À l'opposé, KISS-ICP propose un
pipeline minimaliste et robuste basé sur l'ICP adaptatif, particulièrement
adapté aux plateformes embarquées~\cite{vizzo2023kissICP}.
...
\emph{MonoGS}) ouvre la voie à des cartes denses et photo-réalistes.\cite{kerbl2023gaussian}

\chapter{User Stories}

Les \emph{user stories} suivantes ont été définies par mes soins pour permetre
une meilleure comprension de l'objectif principal et afin de créer un Kanban.
...
\midrule
US-01 & Developpeur & Comprendre l'architecture matérielle et logicielle du TurtleBot~4
       & Haute \\
US-02 & Developpeur & Calibrer la caméra RGB et le LiDAR (intrinsèque +
       extrinsèque)
       & Haute \\
US-03 & Developpeur & Lancer un pipeline SLAM RGB et visualiser la carte en
       temps réel dans RViz2
       & Haute \\
US-04 & Developpeur & Lancer un pipeline SLAM LiDAR et comparer la précision
       avec US-03
       & Haute \\
US-05 & Developpeur & Fusionner les données RGB + LiDAR pour améliorer la
       robustesse du SLAM
       & Moyenne \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}[label=\textbf{BNF-\arabic*}]
  \item \textbf{Performance}~: le pipeline SLAM doit pouvoir lancer soit le SLAM RGB, soit le SLAM LiDAR 
    en temps réel (fréquence d'au moins 5~Hz) sur le User PC.\@
...
\midrule
T-01 & BF-1 & Téléopérer le robot sur 5~m en ligne droite et avoir des retours
      camera stable sur rviz & Déviation $< 5$~cm \\
T-02 & BF-3 & Calibration caméra sur damier & Erreur de reprojection $< 0.4$~px \\
T-03 & BF-4 & SLAM RGB sur séquence corridor 3~m & ATE $< 0.1$~m \\
...
pour le développement. Les choix ont été fait en fonction de s'il etait
compatible avec ROS~2 Jazzy et s'ils etaient facilement integrable dans le projet.

\section{Système d'exploitation et middleware}
Le systeme d'exploitation ici ce fait de soit car pour assurer la bonne continuite du projet et pour eviter les problemes de compatibilité
il faut choisir un systeme d'exploitation compatible avec ROS~2 Jazzy.

\begin{description}
  \item[Ubuntu 22.04 LTS] Distribution de référence pour ROS~2 Jazzy,
    bénéficiant d'un support long terme.
...
Le SLAM a ete implemente ``from scratch'' pour le LiDAR (ICP) et la calibration extrinsèque,
afin de mieux comprendre les algorithmes de base.

\begin{description}
  \item[ICP] Implémenté à partir de zéro pour l'odométrie LiDAR, en utilisant une
    approche de correspondance de points et d'estimation de transformation
    rigide.
  \item[Bresnham] Utilisé pour trouver les correspondances entre les points du LiDAR et les pixels.
  \item[ORB-SLAM3] Utilisé pour la branche SLAM visuel pur (RGB) afin de
    disposer d'une référence académique reconnue.
\end{description}

\section{Calibration}
La calibration de la camera n'as pas ete l'objectif principal du projet alors le 
choix d'utiliser une solution existante a été fait pour gagner du temps et se concentrer sur le SLAM.\@
...
\section{Évaluation}
L'évaluation de la précision de l'odométrie LiDAR est réalisée à l'aide de la
bibliothèque \texttt{rpg\_trajectory\_evaluation} développée par l'Université de Zurich, qui fournit des métriques standardisées (ATE, RPE).

\begin{description}
  \item[rpg\_trajectory\_evaluation] Pour l'evaluation de la precision de l'odométrie LiDAR via \url{https://github.com/uzh-rpg/rpg_trajectory_evaluation}
\end{description}

\section{Développement et versioning}

\begin{description}
  \item[C++] Langage de programmation compatible avec ROS~2 Jazzy.
  \item[Git + GitHub] Versioning et collaboration.
  \item [Gazebo] Simulateur de robotique 3D pour les tests préliminaires en
    simulation avant déploiement sur le robot réel. De plus le turtlebot~4 a une simulation officielle disponible.
  \item[Colcon] Outil de build recommandé pour ROS~2, facilitant la gestion des
    dépendances et l'organisation du workspace.
  \item[Python 3.10 et OpenCV] Langage de programmation utilise pour la pipeline caméra avec
    OpenCV.\@
  \item[C++] Langage de programmation pour le LIDAR ICP, offrant de meilleures performances.
  \item[LLMs (Claude, Gemini)] Utilisés pour la génération de code, la rédaction de documentation
    et l'assistance à la résolution de problèmes.
\end{description}
...
\includegraphics[width=\textwidth]{slam_uml.jpg}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{benchmap.png}
    \caption{Carte SLAM benchmark dans RViz2}\label{fig:rviz_rgb}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{lidarmap.png}
...
\For{chaque keyframe candidate $\mathbf{k}_i$ avec $i < n_{\text{cur}} - A$}
  ...
  \State \textbf{Verrou 1  proximité spatiale.}
         Calculer la distance euclidéenne entre la position courante et la position de la candidate.
...
Ce projet a permis d'améliorer la prise en main de la plateforme TurtleBot~4 ainsi que le 
fonctionement des piplines SLAM en général. La plateforme TurtleBot~4
s'est avérée simple à prendre en main, hors mis la mise en place de l'environnement ROS.
Le pipeline MC-SLAM développé sous ROS~2 montre la difficulté de l'adaptation sur les séquences
testées.
Au final, je ne suis pas content du résultat final, le projet a été plus difficile que prévu et je n'ai pas réussi à faire fonctionner
les pipelines SLAM de manière satisfaisante, que ce soit en RGB ou en LiDAR. Par contre, j'ai
appris énormément sur les algorithmes de SLAM, la calibration multi-capteurs et les défis de l'implémentation sur une plateforme embarquée.
