...
public:
  SlamCoreNode() : Node("slam_core"), last_odom_pose_(Eigen::Vector3d::Zero()) {
    ...
    map_timer_ = create_wall_timer(std::chrono::milliseconds(1000), [this]() {
    ...
    if (use_lidar_) {
      ...
      lidar_loop_sub_ = create_subscription<slam_msgs::msg::LoopConstraint>(
          ...
          std::bind(&SlamCoreNode::onLoopConstraint, this,
                    std::placeholders::_1));
...
private:
  ...
  rclcpp::Subscription<slam_msgs::msg::LoopConstraint>::SharedPtr
      lidar_loop_sub_;
  ...
  void onWheelOdom(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    Eigen::Vector3d cur(msg->pose.pose.position.x, msg->pose.pose.position.y,
                        yaw);
    ...
    double dx_world = cur.x() - last_odom_pose_.x();
    double dy_world = cur.y() - last_odom_pose_.y();
    ...
    double dx_local = co * dx_world + so * dy_world;
    double dy_local = -so * dx_world + co * dy_world;

    double motion = std::sqrt(dx_local * dx_local + dy_local * dy_local);
    Eigen::Matrix3d Q = makeDiag(ODOM_NOISE_XY * motion, ODOM_NOISE_XY * motion,
                                 ODOM_NOISE_TH * std::abs(dth));

    ekf_.predict({dx_local, dy_local, dth}, Q);
  ...
  void onScan(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    PointCloud cloud = parseScan(msg, cur);
    if (cloud.size() < 20)
      return;
  ...
  void
  applyPoseGraphCorrection(const slam_msgs::msg::LoopConstraint::SharedPtr lc) {
    ...
    for (int i = to_id; i < from_id; ++i) {
      ...
      scan_kfs_[i].pose.z() =
          normalizeAngle(scan_kfs_[i].pose.z() + alpha * lc->dtheta);
    }
  ...
  void rayCastScan(const PointCloud &cloud, const Eigen::Vector3d &pose) {
    ...
    for (const auto &pw : cloud) {

      int ex = worldToCell(pw.x(), MAP_ORIGIN_X);
      int ey = worldToCell(pw.y(), MAP_ORIGIN_Y);
  ...
  void publishPose(const rclcpp::Time &stamp) {
    ...
    double co = std::cos(cyaw);
    double so = std::sin(cyaw);
    double cx = mu.x() - (co * ox - so * oy);
    double cy_tf = mu.y() - (so * ox + co * oy);
    ...
    tf.transform.translation.y = cy_tf;
    tf.transform.translation.z = 0.0;
    double half_c = cyaw * 0.5;
    tf.transform.rotation.z = std::sin(half_c);
    tf.transform.rotation.w = std::cos(half_c);
    if (publish_tf_) {
  ...
  PointCloud parseScan(const sensor_msgs::msg::LaserScan::SharedPtr &msg,
    ...
    Eigen::Matrix2f R_robot;
    R_robot << ch, -sh, sh, ch;

    Eigen::Matrix2f R_full = R_robot * lidar_to_base_.linear();
    Eigen::Vector2f t_full = Eigen::Vector2f(static_cast<float>(pose.x()),
                                             static_cast<float>(pose.y())) +
                             R_robot * lidar_to_base_.translation();
    ...
    for (size_t i = 0; i < msg->ranges.size(); ++i) {
      ...
      cloud.push_back(R_full * p_lidar + t_full);
    }
    return cloud;
  }
  ...
  bool fetchLidarTF() {
    try {
      auto tf = tf_buffer_->lookupTransform("base_link", "rplidar_link",
                                            tf2::TimePointZero);
  ...
  poseFromOdom(const nav_msgs::msg::Odometry::SharedPtr &msg) {
    double qx = msg->pose.pose.orientation.x;
    double qy = msg->pose.pose.orientation.y;
    double qz = msg->pose.pose.orientation.z;
    double qw = msg->pose.pose.orientation.w;
