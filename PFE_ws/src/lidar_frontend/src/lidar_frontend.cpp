...
public:
  LidarFrontendNode()
      : Node("lidar_frontend"), robot_pose_(Eigen::Vector3d::Zero()),
    ...
    odom_pub_ =
        create_publisher<nav_msgs::msg::Odometry>("/lidar/odometry", 10);
    loop_pub_ = create_publisher<slam_msgs::msg::LoopConstraint>(
...
private:
  ...
  void onOdom(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    if (!first_odom_) {
      double dx_world = cur.x() - odom_prev_.x();
      double dy_world = cur.y() - odom_prev_.y();
      ...
      double co = std::cos(heading_old);
      double so = std::sin(heading_old);
      double dx_local = co * dx_world + so * dy_world;
      double dy_local = -so * dx_world + co * dy_world;

      double heading_mid =
          heading_old + 0.5 * normalizeAngle(robot_pose_.z() - heading_old);
      double cm = std::cos(heading_mid);
      double sm = std::sin(heading_mid);
      robot_pose_.x() += dx_local * cm - dy_local * sm;
      robot_pose_.y() += dx_local * sm + dy_local * cm;
    }
  ...
  void onScan(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    if (matcher_.hasMap()) {
      ...
      if (res.converged && res.mean_error < 0.10f) {
        ...
        robot_pose_.x() = static_cast<double>(guess.x() + res.t.x());
        robot_pose_.y() = static_cast<double>(guess.y() + res.t.y());
        ...
        RCLCPP_WARN(get_logger(), "ICP failed: converged=%d score=%.3f",
    ...
    double angle =
        std::abs(normalizeAngle(robot_pose_.z() - last_kf_pose_.z()));
  ...
  void detectLoop(const KeyFrame &cur) {
    for (int i = 0; i < cur.id - LC_MIN_AGE; ++i) {
      ...
      if (std::sqrt(dx * dx + dy * dy) > LC_SEARCH_RADIUS)
        continue;
      ...
      if (!res.converged || res.mean_error > LC_ICP_SCORE_THR)
        continue;
      ...
      for (const auto &p_src : cur.cloud) {
        ...
        if (best < LC_INLIER_DIST * LC_INLIER_DIST)
          ++inliers;
      }

      float inlier_ratio =
          static_cast<float>(inliers) / static_cast<float>(cur.cloud.size());
    ...
    try {
      if (!tf_buffer_->canTransform(
        ...
        RCLCPP_WARN(get_logger(),
                    "Waiting for TF base_link -> rplidar_link...");
        return false;
      }
